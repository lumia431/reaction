...
[[nodiscard]] constexpr auto make_binary_expr(L&& l, R&& r) noexcept(
    std::is_nothrow_constructible_v<
        BinaryOpExpr<Op, ExprTraits<std::remove_cvref_t<L>>, ExprTraits<std::remove_cvref_t<R>>>,
        L,
        R>) {
...
#define REACTION_DEFINE_UNARY_OPERATOR(op_symbol, OpType)                                          \
    template <typename T>                                                                          \
    requires IsReact<std::remove_cvref_t<T>> || IsBinaryOpExpr<std::remove_cvref_t<T>> ||          \
             IsUnaryOpExpr<std::remove_cvref_t<T>>                                                 \
