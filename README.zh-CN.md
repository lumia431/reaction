...
与 rxcpp 的性能对比结果 (测试于 2025-06-14):

### 深层依赖测试 (树状结构，深度=10)

| 框架          | 平均时间 (纳秒) | 迭代次数   | 相对速度 |
|--------------|---------------:|-----------:|---------:|
| reaction     |          68.3  |  9,803,835 | **1,663倍** 更快 |
| rxcpp        |     113,571    |      6,290 | (基线)   |

### 宽依赖测试 (10,000 节点)

| 框架          | 平均时间 (纳秒) | 迭代次数   | 相对速度 |
|--------------|---------------:|-----------:|---------:|
| reaction     |     261,448    |      2,626 | **2.76倍** 更快 |
| rxcpp        |     721,404    |        960 | (基线)   |

### 关键发现:
1. **深层依赖场景**: 比 rxcpp 快约 1,663 倍
2. **宽依赖场景**: 比 rxcpp 快约 2.76 倍
3. **测试环境**:
   - 8 核 CPU @ 2.8GHz
   - 32KB L1 数据缓存
   - 4MB L2 统一缓存
