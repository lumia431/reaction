...
TEST(ThreadSafetyTest, SharedDependencyGraphReadOnly) {
    ...
    auto readOnlyOperation = [&](int threadId) {
        try {
            for (int i = 0; i < operationsPerThread; ++i) {
                ...
                // Verify consistency
                if (val3 != val1 + val2) {
                    std::cout << "Read-only consistency violation! "
                              << "expr3=" << val3 << ", expr1=" << val1
                              << ", expr2=" << val2 << std::endl;
                    raceDetected = true;
                }
    ...
    // Verify results
    EXPECT_EQ(operationsCompleted.load(), numThreads * operationsPerThread);
    EXPECT_FALSE(raceDetected.load()) << "Race conditions detected in read-only operations";
...
/**
 * @brief Test basic dependency invalidation (variable change -> expression update)
 */
TEST(ThreadSafetyTest, BasicDependencyInvalidation) {
    std::cout << "=== Basic Dependency Invalidation Test ===" << std::endl;

    // Create var -> expr dependency
    auto var = reaction::var(10);
    auto expr = reaction::calc([&]() {
        std::cout << "Expression evaluated with var=" << var() << std::endl;
        return var() * 2;
    });

    // Debug: check if notification is triggered by manually calling notify
    auto varPtr = var.getPtr();
    std::cout << "Before manual notify: expr=" << expr.get() << std::endl;
    varPtr->notify(true);  // Manually trigger notification
    std::cout << "After manual notify: expr=" << expr.get() << std::endl;

    // Initial state
    std::cout << "Initial evaluation:" << std::endl;
    EXPECT_EQ(var.get(), 10);
    EXPECT_EQ(expr.get(), 20);

    std::cout << "Initial: var=" << var.get() << ", expr=" << expr.get() << std::endl;

    // Change variable
    std::cout << "Changing var to 20..." << std::endl;
    var.value(20);

    // Expression should be automatically updated
    std::cout << "After var change:" << std::endl;
    EXPECT_EQ(var.get(), 20);
    EXPECT_EQ(expr.get(), 40);

    std::cout << "After var change: var=" << var.get() << ", expr=" << expr.get() << std::endl;

    // Change variable again
    std::cout << "Changing var to 5..." << std::endl;
    var.value(5);

    std::cout << "After second var change:" << std::endl;
    EXPECT_EQ(var.get(), 5);
    EXPECT_EQ(expr.get(), 10);

    std::cout << "After second var change: var=" << var.get() << ", expr=" << expr.get() << std::endl;

    std::cout << "✅ Basic dependency invalidation test passed" << std::endl;
}
